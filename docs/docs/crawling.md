...
| `renderer` | string | `auto` | Pin every crawled page to a specific renderer: `auto`, `lightpanda`, `chrome`, `chrome_proxy`, `camoufox`, or `impersonated-http` (self-hosted instances can also configure `playwright`; the managed cloud does not run it). Non-`auto` browser values hard-pin (no fallback) and imply `renderJs:true` unless `renderJs:false` is set; `impersonated-http` executes no JS, so it never implies `renderJs` and `renderJs:true` alongside it is rejected. Validation runs once at crawl start — invalid combinations return HTTP 400 before the job is queued. Per-page failures of a pinned renderer are logged and skipped, so failed pages may be missing from results — see [JS rendering](#js-rendering) for the resilience tradeoff |
